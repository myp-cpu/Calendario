  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      const response = await fetch(`${BACKEND_URL}/api/send-report-email`, {
        ...
        body: JSON.stringify({
          to: emailTo,
          subject: `Reporte ${reportTitle} - ${sectionName} - ${nivelName}`,
          html: htmlContent,
      ...
      if (response.ok) {
        ...
        alert('❌ Error al enviar email: ' + (error.detail || 'Error desconocido'));
      }
    } catch (error) {
          ...
          <div className="flex space-x-3">
            {emailTo && (
              <button
                ...
                className="px-6 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 transition-colors font-medium flex items-center disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {sendingEmail ? (
                  <>
